...
import pickle
import scipy
from GRF_2D_Vmap import Generate_GRF_2D
...
class PI_DeepONet:
    ...
    def residual_net(self, params, u, v, x, y):
        ...
        #print('u_old1 shape=', u_old1.shape, v_old1.shape, vx_old1.shape, vy_old1.shape)
        ...
        #print('NN shape', self.operator_net_u(params_u, u, x, y).shape, s_u_xx.shape)
        ...
        #res0 =0.01 * (s_u_xx + s_v_xy) + 0.01 * s_u_yy
        #res1 =0.01 * (s_v_xx + s_u_xy) + 0.01 * s_v_yy
        return res0, res1
    ...
    def loss_res(self, params, batch):
        ...
        #print('output .shape=',outputs[0][:,index_disk].shape)
        # Compute loss
...
def generate_one_training_data(key, P, Q, N):
    ...
    # Numerical solution # from joblib import Parallel, delayed (incompatible)
    os.chdir(os.path.join(originalDir, './' + '0112_dataload_full_square_with_square_square_129_139_regenerate' + '/'))
    ...
    print('U1d shape=',U1_d.shape, v_down.shape)


    os.chdir(originalDir_real)

    # insert the value into square 

    # import Generate 2D GRF
    # the displacement and velocity for the former time step 
   
    u_tot_l, v_tot_l, vx_tot_l, vy_tot_l = U1_d, V1_d, vx_d, vy_d

    plot_s(X1_.flatten(), Y1_.flatten(), vx_tot_l[0,:], 'vx 1121')
    plot_s(X1_.flatten(), Y1_.flatten(), vy_tot_l[0,:], 'vy 1121')
    plot_s(X1_.flatten(), Y1_.flatten(), u_tot_l[0,:], 'u 1121')
    plot_s(X1_.flatten(), Y1_.flatten(), v_tot_l[0,:], 'v 1121')

    '''plot_s(X1_.flatten(), Y1_.flatten(), vx_tot_p[0], 'vx_p 1121')
    plot_s(X1_.flatten(), Y1_.flatten(), vy_tot_p[0], 'vy_p 1121')
    plot_s(X1_.flatten(), Y1_.flatten(), u_tot_p[0], 'u_p 1121')
    plot_s(X1_.flatten(), Y1_.flatten(), v_tot_p[0], 'v_p 1121')
    plot_s(X1_.flatten(), Y1_.flatten(), vx_tot_n[0], 'vx_n 1121')
    plot_s(X1_.flatten(), Y1_.flatten(), vy_tot_n[0], 'vy_n 1121')
    plot_s(X1_.flatten(), Y1_.flatten(), u_tot_n[0], 'u_n 1121')
    plot_s(X1_.flatten(), Y1_.flatten(), v_tot_n[0], 'v_n 1121')'''
    ...
    plot_s(h_train_l[:m,0], h_train_l[:m,1], u_l[0,:], 'u_l_bc')
    plot_s(h_train_l[:m,0], h_train_l[:m,1], v_l[0,:], 'v_l_bc')
    ...
    v_train = np.hstack([u_tot_l, v_tot_l, vx_tot_l, vy_tot_l, u_bc, v_bc])

    
    # Create grid
    '''print('u_train',u_train.shape)
    test_x = np.transpose(u_train[0, :-2*m].reshape(4, m_i, m_i), (1,2,0))
    #test_x = u_train[0, 0:-2*m].reshape( m_i, m_i, 4)
    #print('u_reshape=',u_train[0, 0:-2*m].reshape(m_i, m_i, 4)[:,:,0] == u_train[0, :m1].reshape(m_i,m_i))
    plot_s(np.transpose(X1.reshape(m_i, m_i)), np.transpose(Y1.reshape(m_i, m_i)), 
           np.transpose(test_x[:,:,0]), ' TEST x1') # this indicates X1 and Y1 are so different transpose should be carefully used 
    plot_s(X1, Y1, u_train[0, :m1].reshape(m_i,m_i),' TEST x2') 
    xx = np.linspace(center[0]-radius, center[0]+radius, m_i)
    yy = np.linspace(center[1]-radius, center[1]+radius, m_i)
    xx, yy = np.meshgrid(xx, yy)
    plot_s(xx, yy, test_x[:,:,0], ' TEST x square')
    test_square  = griddata(np.hstack([X1.reshape(-1,1), Y1.reshape(-1,1)]),
                             u_train[0, :m1], (xx, yy), method='cubic')
    plot_s(xx, yy, test_square, ' TEST x square interpolation')'''
    s_u_train = u_bc
    s_v_train = v_bc
...
def generate_training_data(key, N, P, Q):
    ...
    print('h_r_train', h_r_train[0,:5,:5], np.hstack([X1_.flatten().reshape(-1,1), Y1_.flatten().reshape(-1,1)])[:5,:5])
...
#foldername = 'prepare_DeepONet_disk_dynamic_1115_test_avg_pooling_24_channels_200ptx_400w'
foldername = '0113_prepare_DeepONet_disk_dynamic_square_square_129_139_regenerate'
createFolder(foldername)
os.chdir(os.path.join(originalDir, './' + foldername + '/'))
originalDir_real = os.path.join(originalDir, './' + foldername + '/')

os.chdir(os.path.join(originalDir, './' + '0112_full_square_with_square_square_all_dataset_N100_119_199' + '/'))
# GRF length scale
...
print('up=', X1[index_up])
print('down=', X1[index_down])
print('left=', Y1[index_left])
print('right=', Y1[index_right])
...
'''index_disk = np.where((X1_.flatten() - center[0])**2 + (Y1_.flatten() - center[1])**2 <= radius**2)[0]
index_circle = np.isclose((X1_.flatten() - center[0])**2 + (Y1_.flatten() - center[1])**2, radius**2, 0.02)'''
...
if __name__ == "__main__":
    ...
    #print('s=', u_res_train.shape, s_res_train.shape)
    ...
    with open('DeepONet_DR_129_139_retest.pkl', 'wb') as f:
        pickle.dump(params, f)


    #Plot for loss function
    plot_loss(model.loss_bcs_log, model.loss_res_log)
    #os.chdir(os.path.join(originalDir, './' + 'coupling_elastodynamic_1125_overlapping_simplified_disp_119_120' + '/'))
    #os.chdir(os.path.join(originalDir, './' + 'test_Coupling_1204_translation_10ts_70_81' + '/'))
    #os.chdir(os.path.join(originalDir, './' + '0106_FE_FE_square_disk_L2_error_1e_5' + '/'))
    os.chdir(os.path.join(originalDir, './' + '0113_FE_full_square_square_elasto_dynamic_data_set_119_139_test' + '/'))
    
    #real test
    import numpy as npr # jnp donesn't have loadtxt
    trans = 0.3
    #x_c = np.array(npr.loadtxt('xc1.txt')).reshape(1,-1) - trans
    #y_c = np.array(npr.loadtxt('yc1.txt')).reshape(1,-1) - trans 
    X1_real = npr.loadtxt('X1.txt') #- trans
    Y1_real = npr.loadtxt('Y1.txt') #- trans 

    U1 = npr.loadtxt('U1 ts = 129.txt')
    V1 = npr.loadtxt('V1 ts = 129.txt')
    U1_new = npr.loadtxt('U1 ts = 130.txt')
    V1_new = npr.loadtxt('V1 ts = 130.txt')
    vx = npr.loadtxt('vx ts = 129.txt')/100
    vy = npr.loadtxt('vy ts = 129.txt')/100
    '''u_l = npr.loadtxt('u_l_real.txt')
    v_l = npr.loadtxt('v_l_real.txt')
    u_r = npr.loadtxt('u_r_real.txt')
    v_r = npr.loadtxt('v_r_real.txt')
    u_up = npr.loadtxt('u_up_real.txt')
    v_up = npr.loadtxt('v_up_real.txt')
    u_down = npr.loadtxt('u_low_real.txt')
    v_down = npr.loadtxt('v_low_real.txt')'''
    #print('U1', U1.shape, X1.shape, u_l_bc.shape)
    ...
    print('index_right_real',index_right_real.shape, Y1_real[index_right_real])
    ...
    '''print('up1=', X1[index_up])
    print('down1=', X1[index_down])
    print('left1=', Y1[index_left])
    print('right1=', Y1[index_right])'''

    os.chdir(originalDir_real)


    '''plot_s(X1,Y1, V1, 'test_V1')
    plot_s(X1,Y1, U1, 'test_U1')
    plot_s(X1,Y1, vy, 'test_vy')
    plot_s(X1,Y1, vx, 'test_vx')

    plot_s(X1[index_right], Y1[index_right], u_r, 'test_u_r')
    plot_s(X1[index_right], Y1[index_right], Y1[index_right], 'Y')
    plot_s(X1[index_right], Y1[index_right], U1_new[index_right], 'test_u_r1')
    plot_s(X1[index_up], Y1[index_up], u_up, 'test_u_up_real')
    plot_s(X1[index_up], Y1[index_up], np.flip(u_up), 'test_u_up_real_flip')
    plot_s(X1[index_down], Y1[index_down], u_down, 'test_u_down_real')
    plot_s(X1[index_left], Y1[index_left], u_l, 'test_u_l')
    plot_s(X1[index_right], Y1[index_right], u_r, 'test_u_right')
    plot_s(X1[index_up], Y1[index_up], v_up, 'test_v_up')
    plot_s(X1[index_down], Y1[index_down], v_down, 'test_v_down')
    plot_s(X1[index_left], Y1[index_left], v_l, 'test_v_l')
    plot_s(X1[index_right], Y1[index_right], v_r, 'test_v_right')
    plot_s(X1[index_up], Y1[index_up], U1_new[index_up], 'test_u_up1')'''
    ...
    plot_s(X1_,Y1_, V1_d, 'test_V1_d')
    plot_s(X1_,Y1_, U1_d, 'test_U1_d')

  

    '''y_test_up = np.hstack([X1[index_up].reshape(-1,1), Y1[index_up].reshape(-1,1)])
    y_test_down = np.hstack([X1[index_down].reshape(-1,1), Y1[index_down].reshape(-1,1)])
    y_test_l = np.hstack([X1[index_left].reshape(-1,1), Y1[index_left].reshape(-1,1)])
    y_test_r = np.hstack([X1[index_right].reshape(-1,1), Y1[index_right].reshape(-1,1)])'''



    #u_bc = np.hstack([u_l.reshape(1,-1), u_r.reshape(1,-1), np.flip(u_up.reshape(1,-1)), np.flip(u_down.reshape(1,-1))])
    ...
    plot_bc(np.linspace(0,1,int(u_l.shape[1])),u_l.flatten(),'test u_l_bc1')
    plot_bc(np.linspace(0,1,int(s_v_pred[0,:].shape[0])),s_v_pred[0,:int(v_test[0,:].shape[0])],'test v_l_bc')
    plot_bc(np.linspace(0,1,int(v_l.shape[1])),v_l.flatten(),'test v_l_bc1')
    plot_bc(np.linspace(0,1,int(v_l.shape[1])), s_v_pred[0,:int(v_test[0,:].shape[0])]-v_l.flatten(),'difference v_l_bc')'''
    ...
    plot_s(X1_real, Y1_real, s_u_pred.flatten() - U1_new.flatten(), 's_u_test2_diff')
    plot_s(X1_real, Y1_real, s_v_pred.flatten() - V1_new.flatten(), 's_v_test2_diff')
