...
-----------------------------------------------------------------------
DeepONet Model for elasto-dynamic for time steps 129 to 139
...
def generate_one_training_data(key, P, Q, N):
    ...
    os.chdir(os.path.join(originalDir, './' + 'dataload_from_full_square_dataset_129_139' + '/'))
...
if __name__ == "__main__":
    ...
    foldername = 'prepare_DeepONet_Elasto_dynamic_square_square_129_139'
    ...
    vx = npr.loadtxt('vx ts = 129.txt')/100 # for nomalization velocity should be divided by 100 (as input for CNN)
    vy = npr.loadtxt('vy ts = 129.txt')/100 # for nomalization velocity should be divided by 100 (as input for CNN)
    ...
    # Plot
    plot_disp(X1_real, Y1_real, s_u_pred, 'u_x_pred', rf'$u_{{x, \mathrm{{NO}},\Omega_{{II}}}}^{{{130}}}$')
    plot_disp(X1_real, Y1_real, s_v_pred,'u_y_pred',rf'$u_{{y, \mathrm{{NO}},\Omega_{{II}}}}^{{{130}}}$')
    plot_disp(X1_real, Y1_real,  U1_new.flatten(), 'u_x_truth', rf'$u_{{x, \mathrm{{FE}},\Omega_{{II}}}}^{{{130}}}$')
    plot_disp(X1_real, Y1_real,  V1_new.flatten(), 'u_y_truth', rf'$u_{{y, \mathrm{{FE}},\Omega_{{II}}}}^{{{130}}}$')
    plot_relative_error(X1_real, Y1_real, np.abs(s_u_pred.flatten() - U1_new.flatten()), 'u_x_error',rf'$|u_{{x, \mathrm{{FE}},\Omega_{{II}}}}^{{{130}}} - u_{{x, \mathrm{{NO}},\Omega_{{II}}}}^{{{130}}}|$')
    plot_relative_error(X1_real, Y1_real, np.abs(s_v_pred.flatten() - V1_new.flatten()), 'u_y_error',rf'$|u_{{y, \mathrm{{FE}},\Omega_{{II}}}}^{{{130}}} - u_{{y, \mathrm{{NO}},\Omega_{{II}}}}^{{{130}}}|$')
